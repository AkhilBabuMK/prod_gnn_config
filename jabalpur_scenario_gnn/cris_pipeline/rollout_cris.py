...
class Rollout:
    ...
    def __init__(self, model, ds, day_idx, date, day_meta, freight, disrupt,
                 ...
                 hurdle_tau: float = 1.0, entry_mode: str = 'actual',
                 dep_head: str = 'model', standing_dep: str = 'actual',
                 revise: bool = False, revise_min: float = 0.0):
        ...
        self.standing_dep = standing_dep
        self.revise = revise
        self.revise_min = revise_min
    ...
    def run(self, t0, instances, st_state, prev_delay):
        ...
        takeover, delay_at_t0, standing = {}, {}, {}
        for inst in instances:
            pos = BD._find_position(inst, t0)
            if pos is not None:
                takeover[inst["instance_id"]] = pos["stop_idx"]
                standing[inst["instance_id"]] = pos["mode"] == "at_station"
        ...
        for inst in instances:
            ...
            if k0 is None:
                ...
                # ENTRY MODE. 'actual' hands the rollout the train's TRUE
                # entry time and TRUE entry delay -- both of which lie in the
                # future at T0. Measured on the test days: median 13 min of
                # future information, p90 32 min, 11% over half an hour, and
                # biased (-5 min: trains claw back time on the approach).
                #
                # Training never does this -- build_snapshot drops a train
                # that has not entered, so the weights are clean. It is the
                # simulator alone that is generous.
                #
                # 'timetable' is the opposite extreme: the train is assumed
                # ON TIME to its booked entry with no upstream context at all.
                # Neither mode is what production sees -- production projects
                # entry from the last upstream report before T0 -- but the two
                # BRACKET it, so the gap between them prices the grant before
                # anyone pays for the full projection path.
                if self.entry_mode == "timetable":
                    s0 = inst["stops"][0]
                    if s0.get("sched_arr_abs") is not None:
                        s0["actual_arr_abs"] = float(s0["sched_arr_abs"])
                        s0["actual_dep_abs"] = float(
                            s0.get("sched_dep_abs") or s0["sched_arr_abs"])
                        s0["arr_delay"] = 0.0
                        s0["dep_delay"] = 0.0
                        s0["delay_est_arr"] = 0.0
                        s0["delay_est"] = 0.0
                        # entry_delay_min / upstream_* are all read off the
                        # prefix INCLUDING the part after T0, so they go too.
                        inst["entry"] = {}
            # THE STANDING-DEPARTURE GRANT.
            #
            # The erase below wipes every stop AFTER the one the train
            # occupies. It leaves that stop alone -- correctly for its
            # ARRIVAL, which already happened, but its DEPARTURE has not.
            # `_find_position` reads that departure at every later tick to
            # decide "is this train still on the platform", so the forecast
            # world releases a held train at the exact minute reality did.
            # Measured over trains standing at a decision time: a median of
            # 11 minutes of future knowledge, p90 37, worst 43, on the 22% of
            # forecasts that begin with the train at a platform.
            #
            # Same family as the entry grant, and unlike that one it was never
            # documented. Nothing predicts this departure -- _write_prediction
            # only ever writes stop k+1 -- so closing it needs a rule. The
            # honest one is the timetable: leave when the booked dwell is up,
            # or now if that moment has already passed.
            if (self.standing_dep == "booked"
                    and standing.get(inst["instance_id"])):
                s0 = inst["stops"][k0]
                sa, sd = s0.get("sched_arr_abs"), s0.get("sched_dep_abs")
                if sa is not None and sd is not None:
                    booked = max(0.0, float(sd) - float(sa))
                    dep = max(float(s0["actual_arr_abs"]) + booked, float(t0))
                    s0["actual_dep_abs"] = dep
                    s0["dep_delay"] = dep - float(sd)
        ...
        n_hurdle = [0]
        n_extended = [0]
        # (iid, k) -> index in `records`. A revised prediction must REPLACE
        # its earlier version, never append: appending would count one
        # forecast event many times and quietly reweight the whole metric
        # toward trains that happened to stand the longest.
        rec_at: dict[tuple[str, int], int] = {}
        ...
        for m in ticks:
            ...
            rem_q = out.get("remaining_dwell") if self.revise else None
            ...
            for j, n in enumerate(snap["train_nodes"]):
                ...
                # STANDING: the train is on a platform right now.
                #
                # Two things become possible that were not before.
                #
                # 1. EXTEND THE STAND. A stop's departure is written while the
                #    train is still one stop back, before it has arrived, and
                #    was never revisited -- so a hold beginning after T0 could
                #    not lengthen, the platform freed on schedule, and the
                #    queue behind never formed. `remaining_dwell` answers "how
                #    much longer" from the state on the platform, every tick,
                #    so the train stays because the MODEL keeps saying it will.
                #
                # 2. REVISE THE NEXT ARRIVAL. Today each (train, next stop) is
                #    predicted exactly once, at the first tick the train is
                #    seen -- which for a hold is the one moment the answer is
                #    unknowable. Overwriting while it stands lets the forecast
                #    improve as the evidence arrives, which is what a
                #    controller does.
                #
                # Both stop the moment it departs: once running, the arrival is
                # locked, or predictions would thrash on every tick.
                standing = self.revise and not int(n.get("mode_in_transit", 1))
                if not standing and (iid, k) in written:
                    continue
                ...
                if sp_logit is not None and self.hurdle_tau < 1.0:
                    ...
                    if p_big > self.hurdle_tau:
                        ...
                        n_hurdle[0] += 1
                pred_arr = cur + delta
                # ABLATION. `next_dep` is trained (at half loss weight, 30,977
                # parameters) but is NOT part of val_mae and is never scored
                # against ground truth anywhere -- its only effect is to set
                # departure times inside the forecast world, which decides how
                # long a train occupies a platform and therefore how much
                # congestion it creates for everyone else.
                #
                # `--dep-head booked` forces the extension to zero, i.e. every
                # train dwells exactly as booked. That is the honest control:
                # if the head earns nothing against it, it is 30k parameters
                # and a loss term doing no work.
                dep_q_j = 0.0 if self.dep_head == "booked" else float(dep_q[j])
                # The dep head means different things in the two regimes;
                # composing them wrongly silently corrupts every rollout.
                pred_dep = (pred_arr + dep_q_j
                            if self.dep_mode == 'extension'
                            else cur + dep_q_j)

                # Extend the stand BEFORE writing the next stop, so the
                # monotonicity clamp inside _write_prediction sees the updated
                # departure and can never place the next arrival before it.
                # REVISION FLOOR. Measured on the test days, the head over-
                # predicts exactly where it does the most damage: on stands
                # with under 3 min left it says 3.65 against a truth of 0.81,
                # and those are 1,185 of 3,540 supervised observations. Because
                # a departure may only ever move LATER (below), every one of
                # those over-guesses is permanent while every correction is
                # discarded -- so across 80 ticks nothing leaves on time.
                # Unfiltered revision scored 12.86 against 7.71, worse than
                # persistence.
                #
                # The floor keeps the mechanism only where it was wanted: a
                # hold long enough to block a platform and queue the trains
                # behind it. Below the floor the departure is left exactly as
                # the unrevised rollout wrote it.
                #
                # 0.0 is a no-op -- `rem` is already clamped non-negative, so
                # the comparison is always true and this reproduces the
                # unfiltered behaviour byte for byte.
                if standing and rem_q is not None:
                    s_here = inst["stops"][i]
                    rem = min(max(float(rem_q[j]), 0.0),
                              BD.MAX_REMAINING_DWELL_MIN)
                    if rem >= self.revise_min:
                        new_dep = float(t) + rem
                        # Only ever push a departure LATER. Letting it move
                        # earlier would rewrite history the forecast has
                        # already acted on -- other trains were held against
                        # this platform being busy.
                        if new_dep > float(s_here["actual_dep_abs"]):
                            cap = (float(s_here["actual_arr_abs"])
                                   + BD.MAX_REMAINING_DWELL_MIN)
                            s_here["actual_dep_abs"] = min(new_dep, cap)
                            if s_here.get("sched_dep_abs") is not None:
                                s_here["dep_delay"] = (
                                    s_here["actual_dep_abs"]
                                    - float(s_here["sched_dep_abs"]))
                            n_extended[0] += 1
                ...
                prev = rec_at.get((iid, k))
                if prev is None:
                    rec_at[(iid, k)] = len(records)
                    records.append(row)
                else:
                    records[prev] = row

        if n_extended[0]:
            print(f"    stand extended {n_extended[0]} times", flush=True)
        if n_hurdle[0]:
...
def main() -> None:
    ...
    ap.add_argument("--dep-head", choices=["model", "booked"], default="model",
                    help="'model' uses the trained departure head to set dwell "
                         "inside the forecast world; 'booked' forces every "
                         "dwell to its booked value, ablating the head.")
    ap.add_argument("--revise", action="store_true",
                    help="while a train STANDS at a platform, re-ask how "
                         "much longer it will stay and re-predict its next "
                         "arrival every tick, instead of deciding both once "
                         "at the first tick. Needs a checkpoint with the "
                         "remaining_dwell head.")
    ap.add_argument("--revise-min", type=float, default=0.0,
                    help="minutes: only EXTEND a stand when the head asks for "
                         "at least this long. The head over-predicts short "
                         "stands (3.65 min against a truth of 0.81) and a "
                         "departure may only move later, so unfiltered "
                         "revision holds everything. 0.0 disables the floor "
                         "and reproduces unfiltered --revise exactly. Only "
                         "meaningful with --revise.")
    ap.add_argument("--standing-dep", choices=["actual", "booked"],
                    default="actual",
                    help="when does a train ALREADY STANDING at T0 leave? "
                         "'actual' keeps its true departure (future "
                         "information, the historical default); 'booked' "
                         "releases it after its booked dwell, or at once if "
                         "that moment has passed.")
    ap.add_argument("--entry-mode", choices=["actual", "timetable"],
                    default="actual",
                    help="how trains that enter the corridor AFTER T0 are "
                         "injected. 'actual' = true entry time and delay "
                         "(future information, the historical default); "
                         "'timetable' = booked entry, on time, no upstream "
                         "context. The two bracket what production can know.")
    ...
    # Score a model under the convention it was TRAINED on. v6..v12
    # froze a standing train's delay at its arrival value; v13 lets it grow.
    # Reading this from the checkpoint means an old model can never be
    # accidentally scored under new rules, which would look like a
    # regression that is really a mismatch.
    BD.HONEST_STANDING_DELAY = bool(ck.get("honest_standing_delay", False))
    if BD.HONEST_STANDING_DELAY:
        print("  honest standing delay: ON (v13 convention)")
    hurdle_ok, dwell_ok = load_weights(model, ck["model"])
    # An untrained head sits at its initialisation and emits noise. Letting
    # --revise consult one would extend stands at random and look exactly like
    # a research result, so refuse rather than warn.
    if args.revise and not dwell_ok:
        raise SystemExit(
            "--revise needs a checkpoint trained with the remaining_dwell head; "
            f"{args.checkpoint} predates it. Re-run without --revise, or use a "
            "v7 checkpoint.")
    if not hurdle_ok:
    ...
    # Pre-v6 checkpoints were trained before the two timetable-slack dims, so
    # they expect 44 rather than 46. Must come BEFORE the dataset is built --
    # CrisDataset snapshots the feature width at construction. A no-op for
    # anything trained at v6 or later.
    match_checkpoint_features(ck["config"])

    ds = CrisDataset(args.dataset, args.topology, lazy=True, verbose=False)
    # Built after the model here, so the guard lands here rather than at
    # construction. Still ahead of the first forward pass, which is all
    # that matters -- the alternative is a shape error from inside a
    # Linear, four frames deep, that reads like a model bug.
    ...
    print(f"decision times/day: {len(decisions)}  "
          f"horizon {HORIZON_MIN} min  entry_mode {args.entry_mode}"
          f"  dep_head {args.dep_head}"
          f"  standing_dep {args.standing_dep}"
          f"{'  revise' if args.revise else ''}"
          f"{f'  revise_min {args.revise_min:g}' if args.revise and args.revise_min else ''}\n")
        ...
        for m in BD.SNAPSHOT_SCHEDULE:
            ...
            if m in pending:
                ...
                ro = Rollout(model, ds, di, date, day_meta, freight, disrupt,
                             ...
                             dep_head=args.dep_head,
                             standing_dep=args.standing_dep,
                             revise=args.revise,
                             revise_min=args.revise_min)
    ...
    if later:
        report(later, 0, "ENTERED LATER — " + (
            "true entry delay assumed known" if args.entry_mode == "actual"
            else "booked entry assumed, on time"))
