...
class Rollout:
    ...
    def run(self, t0, instances, st_state, prev_delay):
        ...
        for m in ticks:
            ...
            snap = BD.build_snapshot(self.day_idx, t,
                                     list(self.truth.values()) if TEACHER_FORCED
                                     else instances,
                                     st_state,
            ...
            if not snap["train_nodes"]:
                continue
            data = self.ds.materialise(
                snap, (self.truth if TEACHER_FORCED else by_iid), routes)
